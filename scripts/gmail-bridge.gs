...
function parseMessage_(msg) {
  ...
  var text = msg.getPlainBody() || html.replace(/<[^>]+>/g, " ");
  var links = (html.match(/https?:\/\/brightspace\.cuny\.edu\/d2l\/[^"'\s<>]+/g) || []).map(function (u) { return u.replace(/&amp;/g, "&"); });
  ...
  var body = out.join("\n").replace(/\n{3,}/g, "\n\n").trim();
  // Unwrap Outlook "safe links" (https://urldefense.com/v3/__REAL__;...$) and drop the Brightspace footer.
  body = body.replace(/https?:\/\/urldefense\.com\/v3\/__([^_>\s]+(?:_[^_>\s]+)*)__;[^>\s]*/g, function (all, real) { return real.replace(/\*/g, "#"); });
  body = body.replace(/\n[^\n]*Go to Announcements\s*>>[^\n]*$/i, "").replace(/\n\d{4}\s+[A-Z]{2}\s+\[\d+\][^\n]*<https:\/\/brightspace[^>]*>\s*$/i, "").trim();
  body = body.replace(/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/g, function (addr) { return /csci127@hunter\.cuny\.edu/i.test(addr) ? addr : "[e-mail removed]"; });
  if (body.length > MAX_BODY) body = body.slice(0, MAX_BODY).replace(/\s+\S*$/, "") + " …";
  ...
  return { key: key, title: title.slice(0, 120), date: date, text: body, link: newsLink };
}

function postIssue_(a) {
  var tok = token_();
  if (!tok) { Logger.log("GITHUB_TOKEN missing in Script properties."); return null; }
  var issueBody = "### Date\n\n" + a.date + "\n\n### What it says\n\n" + a.text + "\n\n### Link\n\n" + (a.link || "_No response_") + "\n\n_Posted automatically from a Brightspace notification e-mail._";
